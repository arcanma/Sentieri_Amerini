...
L.Control.PanelLayers = L.Control.Layers.extend({
	...
	_updateHeight: function (h) {
		...
		if (this.options.compact)
			this._form.style.maxHeight = h-28 + 'px'; /* Marcello Arcangeli: changed from h + 'px' to h-28 +'px' to avoid panel layer 
														under the map attribution, causing problem to select last menu item */
		else
			this._form.style.height = h-28 + 'px'; /* Marcello Arcangeli: changed from h + 'px' to h-28 +'px' to avoid panel layer 
														under the map attribution, causing problem to select last menu item */
		},
